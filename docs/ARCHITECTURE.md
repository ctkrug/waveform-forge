...
File (drag/drop or <input type=file>)
  ...
  -> SelectionPlayer             (src/audio/player.ts)          -> live preview + playhead + peak level
...
| `src/lib/meter.ts`           | `amplitudeToDb`/`dbToMeterRatio`/`isClipping` — the peak-level meter's linear-amplitude -> dB -> 0..1 fill-ratio scale, and its clip threshold.                                                                                    |
...
| `src/audio/ffmpeg-client.ts` | Lazy-loaded ffmpeg.wasm singleton; `demuxToWav` (decode fallback) and `transcode` (export), both wired for the ~30MB core to load only on first use and serialized through `withFfmpegLock` so their `exec()` calls never overlap. |
...
| `src/audio/player.ts`        | `SelectionPlayer` — plays a trim selection via `AudioBufferSourceNode`, optionally looped, exposes a clock-derived `currentTime()` and an `AnalyserNode`-backed `peakLevel()` for the playhead and level meter.                    |
...
| `src/ui/level-meter.ts`      | `LevelMeter` — sets the peak-meter fill width and latches its clip LED on for 1.5s after the last clipping sample.                                                                                                                 |
